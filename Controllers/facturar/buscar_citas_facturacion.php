...
$id_paciente = $_POST['id_paciente'];  // Ya está en formato yyyy-mm-dd
...
try {
    ...
    $citas_datos = $citas->searchCitasNoPagasPorPacienteNoFacturadas($id_paciente, $desde, $hasta);
    
    session_start();
    // Asegurarse de que $citas_datos tiene los datos que esperamos
    if (empty($citas_datos)) {
        
        $_SESSION['mensaje']= "No se encontraron citas no pagadas no facturadas.";
        $_SESSION['icono'] = "error";
        header('Location: ../../view/citas/index.php');
        exit;
    }
    ...
    foreach ($citas_datos as &$cita) { 
        ...
        header('Location: ../../view/facturar/listado_impagos.php');
