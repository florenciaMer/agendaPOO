...
try {
    ...
    $citas_datos = $citas->searchCitasNoFacturadasPorPaciente($id_paciente, $desde, $hasta);
    var_dump($citas_datos);
    session_start();
    // Asegurarse de que $citas_datos tiene los datos que esperamos
    if (empty($citas_datos)) {
        echo json_encode(['error' => 'No se encontraron citas no facturadas .']);
        exit;
    }
